...
impl ToolRegistry {
    ...
    pub fn detect_and_execute(&self, text: &str) -> Option<ToolResult> {
        ...
        if t.starts_with("复制") {
            return Some(self.clip_set(t));
        }
        // 6. 翻译
        if ["翻译", "translate", "用英语说", "用日语说", "翻译成"]
            .iter()
            .any(|k| t.contains(k))
        {
    ...
    fn calc(&self, expr: &str) -> ToolResult {
        ...
        match self.eval(&e) {
            Ok(r) => {
                let s = if r.fract() == 0.0 && r.abs() < 1e15 {
                    format!("{}", r as i64)
                } else {
                    format!("{:.4}", r)
                        .trim_end_matches('0')
    ...
    fn sys_cmd(&self, t: &str) -> ToolResult {
        let (cmd, desc) = if t.contains("打开浏览器") || t.contains("打开网页") {
            ("cmd /c start https://www.baidu.com", "打开浏览器")
        } else if t.contains("打开计算器") || t.contains("计算器") {
            ("cmd /c calc", "打开计算器")
        } else if t.contains("打开记事本") || t.contains("记事本") {
            ("cmd /c notepad", "打开记事本")
        } else if t.contains("打开文件管理器") || t.contains("打开文件夹") {
            ("cmd /c explorer", "打开文件管理器")
        } else if t.contains("打开微信") {
            ("cmd /c start WeChat:", "打开微信")
        } else if t.contains("打开 VS Code") || t.contains("打开编辑器") {
            ("cmd /c code", "打开 VS Code")
        } else if t.contains("锁屏") {
            ("rundll32.exe user32.dll,LockWorkStation", "锁屏")
        } else if t.contains("关机") {
            ("shutdown /s /t 60", "60秒后关机")
        } else if t.contains("取消关机") {
            ("shutdown /a", "取消关机")
        } else if t.contains("重启") {
            ("shutdown /r /t 60", "60秒后重启")
        } else if t.contains("休眠") {
            ("rundll32.exe powrprof.dll,SetSuspendState 0,1,0", "休眠")
        } else {
            return ToolResult::simple(
                "我支持：打开浏览器/计算器/记事本/文件夹/微信/VS Code、锁屏、关机、重启、休眠",
            );
        };
    ...
    fn clip_get(&self) -> ToolResult {
        match self.read_clip() {
            Ok(t) => {
                if t.is_empty() {
                    ToolResult::simple("剪贴板是空的")
                } else {
                    if let Some(ref db) = self.db {
                        let db = db.lock().unwrap();
                        let _ = db.save_memory(&t, "clipboard", Some("system"), None, 0.3);
                    }
    ...
    fn clip_set(&self, t: &str) -> ToolResult {
        ...
        if c.is_empty() {
            return ToolResult::simple("你想复制什么？");
        }
    ...
    fn write_clip(&self, t: &str) -> Result<(), String> {
        std::process::Command::new("powershell")
            .args([
                "-NoProfile",
                "-Command",
                &format!("Set-Clipboard -Value '{}'", t.replace('\'', "''")),
            ])
            .output()
            .map_err(|e| e.to_string())?;
        Ok(())
        ...
        if let Ok(cur) = self.read_clip() {
            ...
            if !cur.is_empty() && cur != *last && cur.len() > 5 {
                ...
                *last = cur;
                if let Some(ref db) = self.db {
                    let db = db.lock().unwrap();
                    let _ = db.save_memory(&ch, "clipboard", Some("system"), None, 0.3);
                }
                return Some(ch);
            }
        }
    ...
    fn translate(&self, t: &str) -> ToolResult {
        ...
        let text = if let Some(pos) = t.find("翻译成英语") {
            t.get(pos..)
                .unwrap_or("")
                .trim_start_matches("翻译成英语")
                .trim()
        } else if let Some(pos) = t.find("翻译成中文") {
            t.get(pos..)
                .unwrap_or("")
                .trim_start_matches("翻译成中文")
                .trim()
        } else if let Some(pos) = t.find("翻译成日语") {
            t.get(pos..)
                .unwrap_or("")
                .trim_start_matches("翻译成日语")
                .trim()
...
"#,
    ...
    fn schedule_task(&self, t: &str) -> ToolResult {
        ...
        let (interval, cmd) = if let Some(pos) = t.find("每隔") {
            ...
            let unit = if rest.contains("小时") {
                3600
            } else if rest.contains("秒") {
                1
            } else {
                60
            };
        ...
        if t.contains("取消") {
            ...
            if self.scheduled_tasks.is_empty() {
                ...
                let list: Vec<String> = self
                    ...
                    .iter()
                    .enumerate()
                    .map(|(i, t)| format!("{}. {} (每{}秒)", i + 1, t.command, t.interval_secs))
                    .collect();
    ...
    fn list_phrases(&self) -> ToolResult {
        let list: Vec<String> = self
            ...
            .iter()
            .enumerate()
            .map(|(i, p)| format!("{}. {} → \"{}\"", i + 1, p.name, p.phrase))
            .collect();
        ToolResult::simple(&format!(
            "快捷短语：\n{}\n\n说\"执行快捷1\"即可触发",
            list.join("\n")
    ...
    fn score_conversation(&self, t: &str) -> ToolResult {
        let score = if t.contains("五星") || t.contains("好评") || t.contains("👍") {
            5
        } else if t.contains("四星") {
            4
        } else if t.contains("三星") || t.contains("一般") {
            3
        } else if t.contains("二星") || t.contains("差评") {
            2
        } else if t.contains("一星") || t.contains("👎") {
            1
        } else {
            3
        };
    ...
    fn web_search(&self, t: &str) -> ToolResult {
        ...
        let q = q.trim();
        if q.is_empty() {
            return ToolResult::simple("你想搜索什么？");
        }
        ...
        match self.search_ddg(q) {
            Ok(r) => {
                if r.is_empty() {
                    ToolResult::simple(&format!("没找到「{}」的结果", q))
                } else {
                    ToolResult::simple(&r)
                }
            }
            ...
            if l.contains("result-link") || l.contains("result__a") {
                if let Some(s) = l.find("href=\"") {
                    ...
                    if let Some(_e) = r.find('"') {
                        let txt = if let Some(ts) = r.find('>') {
                            if let Some(te) = r[ts..].find("</a>") {
                                r[ts + 1..ts + te]
                                    .trim()
                                    .replace("<b>", "")
                                    .replace("</b>", "")
                        ...
                        if !txt.is_empty() && res.len() < 5 {
                            res.push(format!("{}. {}", res.len() + 1, txt));
                        }
                    }
                }
            }
    ...
    fn export_conv(&self) -> ToolResult {
        let Some(ref db) = self.db else {
            return ToolResult::simple("数据库不可用");
        };
        let db = db.lock().unwrap();
        let dir = dirs::data_local_dir()
            .unwrap_or_default()
    ...
    fn set_reminder(&self, t: &str) -> ToolResult {
        let Some(ref db) = self.db else {
            return ToolResult::simple("提醒不可用");
        };
    ...
    fn list_reminders(&self) -> ToolResult {
        ...
        match db.get_pending_reminders() {
            Ok(rs) => {
                if rs.is_empty() {
                    ToolResult::simple("没有提醒")
                } else {
                    let mut l = vec![format!("{}个提醒：", rs.len())];
                    for r in rs.iter().take(5) {
                        l.push(format!("- {}: {}", r.1, r.2));
                    }
    ...
    fn cancel_reminders(&self) -> ToolResult {
        ...
        match db.get_pending_reminders() {
            Ok(rs) => {
                let c = rs.len();
                for r in &rs {
                    let _ = db.fire_reminder(r.0);
                }
                ToolResult::simple(&format!("已取消{}个提醒", c))
            }
            Err(e) => ToolResult::simple(&format!("失败: {}", e)),
        }
    }
    ...
    fn remember(&self, t: &str) -> ToolResult {
        ...
        for k in &kws {
            if let Some(p) = t.find(k) {
                c = t[p + k.len()..].trim().to_string();
                break;
            }
        }
        if c.is_empty() {
            return ToolResult::simple("你想让我记住什么？");
        }
        let Some(ref db) = self.db else {
            return ToolResult::simple("记忆不可用");
        };
        let db = db.lock().unwrap();
    ...
    fn search_conversation(&self, t: &str) -> ToolResult {
        ...
        let q = q.trim();
        if q.is_empty() {
            return ToolResult::simple("你想搜索什么对话？");
        }
    ...
    fn analyze_emotion(&self, t: &str) -> ToolResult {
        ...
        // 简单的关键词情感分析
        let (emotion, score) = if text.contains("开心")
            || text.contains("高兴")
            || text.contains("快乐")
            || text.contains("太好了")
        {
